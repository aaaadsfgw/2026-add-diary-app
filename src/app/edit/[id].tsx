...
export default function EditEntry() {
  ...
  const { getEntry, loading, updateEntry } = useEntries();
  ...
  const handleSave = async () => {
    if (!entry || !canSave || saving) return;
    setSaving(true);
    setError(null);
    try {
      const saved = await updateEntry(entry.id, { icon, title, body, date, imageUrl });
    ...
    <SafeAreaView style={styles.safe} edges={['top', 'bottom']}>
      ...
      {!entry ? (
        <View style={styles.center}>
          {loading ? <ActivityIndicator color="#5C7D6B" /> : null}
          <Text style={styles.centerText}>
            {loading ? '日記を読み込んでいます' : '日記が見つかりません'}
          </Text>
        </View>
      ) : (
        <KeyboardAvoidingView
